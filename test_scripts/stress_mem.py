...
memory_lines = subprocess.check_output(
    ['free -h --giga | cut -c 1-50'], stderr=subprocess.STDOUT, shell=True).decode('utf8').replace(
        'total    ', '    Всего').replace('used      ', 'Использовано').replace(
            'free', 'Свободно').replace('Mem:', 'Память: ').replace(
                'Swap:', 'Подкачка:').split('\n')
...
def free_mem():
    ...
    while True:
        ...
        print(subprocess.check_output(
            ['free -h --giga | cut -c 1-50'], stderr=subprocess.STDOUT, shell=True).decode(
                'utf8').replace('total    ', '    Всего').replace(
                    'used      ', 'Использовано').replace('free', 'Свободно').replace(
                        'Mem:', 'Память: ').replace('Swap:', 'Подкачка:'))
...
def testing_mem():
    ...
    print(f"\n[{datetime_func()}] Тестирование памяти (memtester)...")
...
def stress_mem_func():
    ...
    if error_continue is True:
        ...
        input_err(name='памяти (Memtester)')
